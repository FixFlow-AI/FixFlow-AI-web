...
import { FileText, Cpu, AlertTriangle, CheckCircle, RefreshCw } from 'lucide-react'
...
export function BriefIntelligence() {
  ...
  const riskLabel = stabilityScore < 50 ? 'HIGH SCOPE CREEP RISK' : stabilityScore < 80 ? 'MODERATE RISK' : 'PREMIUM SCOPE'

  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Title */}
      <div>
        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Subsystem 01</span>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
          <FileText className="text-[#2563EB]" /> Brief Intelligence Workspace
        </h1>
        <p className="text-slate-500 text-sm mt-1">
          Input your raw project briefs to automatically structure deliverables, dependencies, and risk scores.
        </p>
      </div>
        ...
        <div className="bg-white border border-[#D9E0E8] p-6 rounded-lg space-y-6">
          ...
          <form onSubmit={handleParse} className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="brief" className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
        ...
        <div className="bg-white border border-[#D9E0E8] p-6 rounded-lg">
          ...
          {!isBriefParsed && !parsing ? (
            <div className="h-64 flex flex-col items-center justify-center text-slate-400 text-center space-y-2">
              ...
              <p className="text-sm">Click "Parse Project Brief" to run the system scoping parser.</p>
            </div>
          ) : parsing ? (
            <div className="h-64 flex flex-col items-center justify-center text-slate-500 text-center space-y-4">
              ...
              <div className="text-sm space-y-1">
                <p className="font-bold text-slate-800">Autonomous Parser Running</p>
                <p className="text-xs text-slate-400">Reviewing grammar, identifying dependencies, and tagging unknowns.</p>
              </div>
            </div>
          ) : (
            <div className="space-y-6 animate-fadeIn py-2">
              ...
              <div className="p-4 border border-[#D9E0E8] rounded bg-[#F7F8FA] flex justify-between items-center">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Scope Stability Rating</span>
                  <div className="text-xl font-extrabold text-slate-900 mt-1">{stabilityScore}%</div>
                </div>
                <div className="text-right">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Risk Label</span>
                  <div
              ...
              {/* Parsed Outcomes */}
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Parsed Outcomes</h4>
                <ul className="text-sm text-slate-600 space-y-1.5 list-disc pl-4">
                  <li>Configure virtual routing accounts for split-payout mapping.</li>
                  <li>Build a secondary Web3 transaction dispatcher utilizing USDC on Polygon.</li>
                  {isAmbiguous ? (
                    <li className="text-[#C2410C] font-semibold list-none pl-0 mt-1 flex items-center gap-1">
                      <AlertTriangle size={14} /> Gaps found: Describe the subscription system for detailed outcomes.
                    </li>
                  ) : (
                    <>
                      <li>Integrate Stripe-compatible legacy webhooks for backup verification.</li>
                      <li>Deploy automated rollback triggers on sync discrepancies.</li>
                    </>
                  )}
                </ul>
              </div>
              ...
              <div className="space-y-3 pt-3 border-t border-[#D9E0E8]">
                <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1 text-orange-600">
                  <AlertTriangle size={14} /> Clarification Gaps ({answersSubmitted ? '0' : '2'} Gaps Open)
                </h4>

                {answersSubmitted ? (
                  <div className="p-3 bg-emerald-50 border border-emerald-200 text-[#16A34A] rounded text-xs">
                    All clarifications answered. Scope stability rating upgraded.
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label htmlFor="q1" className="block text-xs font-semibold text-slate-600">
                        1. What is the expected maximum latency for Web3 deposit recognition?
                      </label>
                      <input
                        id="q1"
                        type="text"
                    ...
                    <div className="space-y-1.5">
                      <label htmlFor="q2" className="block text-xs font-semibold text-slate-600">
                        2. Do you require real-time email alerts for customer webhook failures?
                      </label>
                      <input
                        id="q2"
                        type="text"
