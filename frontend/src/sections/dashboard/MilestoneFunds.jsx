...
function calculateEarningsBreakdown(grossAmount, plan) {
  const platformRates = { FREE: 0.1, SOLO: 0.05, PRO: 0.03, AGENCY: 0.02 };
  const platformRate = platformRates[plan] || 0.1;
  ...
  const netFreelancerEarnings =
    grossAmount - platformFee - paymentGatewayFee - withholdingTax;
...
export function MilestoneFunds() {
  const { milestones, fundMilestone, releaseMilestone, isAgreementSigned } =
    useLandingStore();
  ...
  const plan = "PRO"; // Mocking PRO plan tier
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Lock deposits in secure multi-sig escrows, review transparent fee
          splits, and authorize releases safely.
        </p>
      </div>
      ...
      {!bothSigned ? (
        <div className="p-8 border border-orange-200 bg-orange-50 rounded-lg text-center max-w-lg mx-auto space-y-4">
          ...
          <p className="text-slate-600 text-sm leading-relaxed">
            The escrow ledger cannot process fund routing until both the Client
            and Freelancer sign the Working Agreement.
          </p>
        </div>
      ) : (
              ...
              {milestones.map((m) => (
                ...
                >
                  <div className="flex justify-between items-center w-full">
                    <span className="font-bold text-slate-800 text-xs leading-normal">
                      {m.title}
                    </span>
                  </div>
              ...
              <>
                ...
                {showMfaInput && (
                  <form
                    ...
                    className="p-4 border border-blue-200 bg-[#EDF4FF] rounded text-xs space-y-3 animate-slideDown"
                  >
                    ...
                    <p className="text-slate-600">
                      ...
                      authenticator app to authorize smart-contract release. For
                      prototype walkthrough, input{" "}
                      <strong className="text-slate-900">123456</strong>.
                    </p>
                    {mfaError && (
                      <p className="text-[#C2410C] font-semibold">{mfaError}</p>
                    )}
                    <div className="flex gap-2">
                      <input
                        type="text"
                  ...
                  <div className="border border-[#D9E0E8] rounded divide-y divide-[#D9E0E8] font-mono text-[11px] text-slate-600">
                    <div className="p-2.5 flex justify-between">
                      ...
                      <span className="font-bold text-slate-900">
                        ${breakdown.grossAmount.toLocaleString()} USDC
                      </span>
                    </div>

                    <div className="p-2.5 flex justify-between bg-slate-50">
                      <span>
                        Platform Commission ({plan === "PRO" ? "3%" : "5%"} cut)
                      </span>
                      <span className="text-[#C2410C] font-semibold">
                        -${breakdown.platformFee.toLocaleString()} USDC
                      </span>
                    </div>
                    ...
                    <div className="p-2.5 flex justify-between">
                      ...
                      <span className="text-[#C2410C] font-semibold">
                        -${breakdown.paymentGatewayFee.toLocaleString()} USDC
                      </span>
                    </div>
                    ...
                    <div className="p-2.5 flex justify-between bg-slate-50">
                      ...
                      <span className="text-[#C2410C] font-semibold">
                        -${breakdown.withholdingTax.toLocaleString()} USDC
                      </span>
                    </div>
                    ...
                    <div className="p-2.5 flex justify-between font-sans font-bold text-emerald-700 text-xs bg-emerald-50">
                      ...
                      <span>
                        ${breakdown.netFreelancerEarnings.toLocaleString()} USDC
                      </span>
                    </div>
                    ...
                    <div className="p-2.5 flex justify-between font-sans font-bold text-slate-700 text-xs">
                      ...
                      <span>
                        ${breakdown.totalClientCheckout.toLocaleString()} USDC
                      </span>
                    </div>
                  </div>
                </div>
