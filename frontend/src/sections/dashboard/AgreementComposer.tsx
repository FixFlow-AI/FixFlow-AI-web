...
import { FileSignature, CheckCircle, ShieldAlert, Edit3, Save } from 'lucide-react'
...
export function AgreementComposer() {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Title */}
      <div>
        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Subsystem 04</span>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
          <FileSignature className="text-[#2563EB]" /> Working Agreement Composer
        </h1>
        <p className="text-slate-500 text-sm mt-1">
          Harden proposal terms into scoped milestones with binding acceptance criteria and escrow release rules.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-3">
            {milestones.map((m) => (
              ...
              >
                ...
                <span className="font-bold text-slate-700 font-mono text-[10px]">${m.amount.toLocaleString()}</span>
              </button>
            ))}
          </div>
          ...
          {activeMilestone && (
            <div className="pt-4 border-t border-[#D9E0E8] space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">Inspect Milestone</span>
              ...
              {isEditing ? (
                <div className="space-y-3">
                  <div className="space-y-1">
                    <label htmlFor="m-title" className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Milestone Title</label>
                    <input
                      id="m-title"
                      type="text"
                      value={editingTitle}
                  ...
                  <div className="space-y-1">
                    <label htmlFor="m-amount" className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Scope Budget (USDC)</label>
                    <input
                      id="m-amount"
                      type="number"
                      value={editingAmount}
                ...
                <div className="space-y-2 text-xs text-slate-600 leading-relaxed bg-[#F7F8FA] p-4 rounded border border-slate-100">
                  <p><strong>Title:</strong> {activeMilestone.title}</p>
                  <p><strong>Escrow Value:</strong> ${activeMilestone.amount.toLocaleString()} USDC</p>
                  <p className="mt-2 text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Agreed Release Checklists:</p>
        ...
        <div className="bg-white border border-[#D9E0E8] p-6 rounded-lg lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4 text-xs text-slate-600 bg-slate-50 p-6 rounded border border-slate-200 leading-relaxed font-sans">
            <h3 className="text-sm font-bold text-slate-800 border-b border-slate-200 pb-2 mb-3">COVENANT OF WORK AGREEMENT</h3>
            <p>
              This document legally binds the client and freelancer under the terms of the Northstar Billing Migration project.
              Funds deposited in the virtual smart escrow contract are subject to strict release checkpoints.
            </p>
            <p className="font-semibold text-slate-800 mt-3">Active Escrow Milestones:</p>
            <ol className="list-decimal pl-4 space-y-1.5 mt-1 font-mono text-[11px]">
              {milestones.map((m) => (
                <li key={m.id}>
                  {m.title} — <span className="font-bold">${m.amount.toLocaleString()} USDC</span>
                </li>
              ))}
            </ol>
            ...
            {bothSigned && (
              <div className="mt-4 pt-3 border-t border-slate-200 space-y-1 font-mono text-[10px] text-slate-500">
                <p>Escrow State Machine Address: <span className="text-slate-700 font-semibold">0x2b8d96e5782782b6c69f2e463a5f782737ef39ce</span></p>
                <p>Polygon Network Registry Status: <span className="text-emerald-600 font-semibold">ACTIVE</span></p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-[#D9E0E8] pt-6">
            <div className="p-4 border border-[#D9E0E8] rounded space-y-3 bg-[#F7F8FA] flex flex-col justify-between h-32">
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Client Authentication</span>
                <span className="text-xs font-bold text-slate-800 mt-1 block">Northstar Org Officer</span>
              </div>
            ...
            <div className="p-4 border border-[#D9E0E8] rounded space-y-3 bg-[#F7F8FA] flex flex-col justify-between h-32">
              <div>
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Freelancer Authentication</span>
                <span className="text-xs font-bold text-slate-800 mt-1 block">Match Candidate</span>
              </div>
          ...
          {bothSigned ? (
            <div className="p-3 bg-emerald-50 border border-emerald-200 text-[#16A34A] rounded text-xs flex items-center gap-2">
              <CheckCircle size={16} /> Both parties signed. Escrow is locked. Ready to deposit capital.
            </div>
          ) : (
            <div className="p-3 bg-orange-50 border border-orange-200 text-[#C2410C] rounded text-xs flex items-center gap-2">
              <ShieldAlert size={16} /> Waiting for signatures before escrow state transitions can execute.
            </div>
          )}
        </div>
      </div>
