...
import { GitBranch, ShieldCheck, GitCommit, FileCode, CheckCircle, ExternalLink } from 'lucide-react'

interface RequirementProof {
  id: string
  title: string
  confidence: number
  status: 'High' | 'Moderate' | 'Open'
  desc: string
  repo: string
  commitHash: string
  commitMsg: string
...
const mockProofs: RequirementProof[] = [
  {
    id: 'req1',
    ...
    commitHash: '8f0a23d',
    commitMsg: 'impl: cache webhook signature and hash matching for 24h replay protection',
    rationale: 'The developer has pushed production code that implements a Redis-backed deduplication sliding window matching this requirement.',
  },
  {
    id: 'req2',
    ...
    commitHash: 'c4e9912',
    commitMsg: 'feat: add virtual routing destination split path mapping to Polygon gateway',
    rationale: 'Direct evidence found in repository containing smart contracts deployed on Polygon Amoy. Audit stamp verified.',
  },
  {
    id: 'req3',
    ...
    rationale: 'Documentation evidence exists in the repository, but actual transactional execution script tests are missing.',
  },
]

export function EvidenceConfidence() {
  const [selectedReq, setSelectedReq] = useState<RequirementProof>(mockProofs[0])

  return (
    <div className="space-y-8 animate-fadeIn">
      {/* Title */}
      <div>
        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Subsystem 02</span>
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
          <GitBranch className="text-[#2563EB]" /> Evidence and Confidence Map
        </h1>
        <p className="text-slate-500 text-sm mt-1">
          Compare candidate capability based on verified repository history rather than profile bidding pitches.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {mockProofs.map((req) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <h4 className="font-bold text-slate-800 text-xs leading-normal">{req.title}</h4>
                  <span
                ...
                <p className="text-[11px] text-slate-500 mt-2 line-clamp-2">{req.desc}</p>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white border border-[#D9E0E8] p-6 rounded-lg lg:col-span-2 space-y-6">
          <div className="text-xs font-bold text-slate-700 uppercase tracking-wider border-b border-[#D9E0E8] pb-2 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-semibold">{selectedReq.title}</span>
          </div>
          ...
          <div className="relative border border-[#D9E0E8] rounded bg-[#F7F8FA] p-6 h-60 flex items-center justify-between overflow-hidden">
            ...
            <div className="z-10 bg-white border-2 border-blue-500 rounded p-3 text-center shadow-sm w-44">
              <span className="text-[10px] font-bold text-blue-600 block uppercase tracking-wider">Requirement</span>
              <span className="text-xs font-bold text-slate-900 line-clamp-1 mt-1">{selectedReq.title}</span>
            </div>
            ...
            <div className="z-10 bg-white border-2 border-emerald-500 rounded p-3 text-center shadow-sm w-44">
              ...
              <span className="text-xs font-bold text-slate-900 line-clamp-1 mt-1">{selectedReq.commitHash}</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Requirement Target</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-[#D9E0E8]">
              <div className="space-y-1">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block flex items-center gap-1">
                  <FileCode size={14} className="text-slate-400" /> Repository Source
                </span>
                <a
              ...
              <div className="space-y-1">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block flex items-center gap-1">
                  <GitCommit size={14} className="text-slate-400" /> Verifiable Commit
                </span>
            ...
            <div className="space-y-1 bg-slate-50 p-4 border border-slate-100 rounded">
              <span className="text-xs font-bold text-slate-700 uppercase tracking-wider block flex items-center gap-1">
                <ShieldCheck size={14} className="text-emerald-500" /> Vetting Recommendation
              </span>
              <p className="text-xs text-slate-600 mt-1 leading-relaxed">{selectedReq.rationale}</p>
            </div>
          </div>
        </div>
      </div>
