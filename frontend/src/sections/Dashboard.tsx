...
export function Dashboard() {
  const { dashboardTab, setDashboardTab, logout, resetMockData } = useLandingStore()

  // Sidebar navigation configuration
  const menuItems = [
    { id: 'overview' as DashboardTabId, label: 'Overview', icon: LayoutDashboard },
    { id: 'brief-intelligence' as DashboardTabId, label: 'Brief Ingestion', icon: FileText },
    { id: 'evidence-confidence' as DashboardTabId, label: 'Evidence Graph', icon: GitBranch },
    { id: 'proposal-generator' as DashboardTabId, label: 'Proposal Builder', icon: Cpu },
    { id: 'agreement-composer' as DashboardTabId, label: 'Agreement Composer', icon: FileSignature },
    { id: 'delivery-control' as DashboardTabId, label: 'Delivery Control', icon: KanbanSquare },
    { id: 'milestone-funds' as DashboardTabId, label: 'Milestone Funds', icon: Coins },
    { id: 'outcome-evidence' as DashboardTabId, label: 'Outcome Reputation', icon: Award },
    { id: 'role-onboarding' as DashboardTabId, label: 'Role Setup', icon: Settings },
  ]
      ...
      <div className="flex-1 md:pl-64 flex flex-col min-h-screen">
        ...
        <header className="bg-white border-b border-[#D9E0E8] h-16 flex items-center justify-between px-8 sticky top-0 z-20">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Project ID:</span>
          ...
          <div className="flex items-center gap-4 text-xs">
            <span className="text-slate-400 font-medium">Consensus network:</span>
