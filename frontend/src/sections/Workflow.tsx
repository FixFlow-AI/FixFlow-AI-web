...
import { BadgeCheck, CircleDollarSign, FileText, Link2, ShieldCheck, UserRound } from 'lucide-react'
import { SectionHeading } from '../components/SectionHeading'
import { workflowPhases, type WorkflowPhaseId } from '../data/landing'
import { useLandingStore } from '../store/useLandingStore'

const lanes: Array<{ label: string; icon: typeof UserRound; active: WorkflowPhaseId[] }> = [
  { label: 'Client', icon: UserRound, active: ['brief', 'agreement', 'approval'] },
  { label: 'FixFlowAI', icon: ShieldCheck, active: ['brief', 'match', 'agreement', 'outcome'] },
  { label: 'Talent', icon: BadgeCheck, active: ['match', 'agreement', 'build', 'approval'] },
  { label: 'Escrow', icon: CircleDollarSign, active: ['agreement', 'build', 'approval', 'outcome'] },
]
  ...
  return (
    <section className="workflow section-band" id="workflow">
      <div className="section-shell">
        ...
        <div className="phase-control" role="tablist" aria-label="Project phases">
          ...
          <div className="workflow-canvas" aria-label="Shared project swimlane">
            <div className="workflow-axis" aria-hidden="true">
              ...
              {workflowPhases.map((item) => (
                <strong className={phase === item.id ? 'is-active' : ''} key={item.id}>{item.label}</strong>
              ))}
            </div>
            {lanes.map((lane) => {
            ...
            <div
              ...
              style={{ '--phase-index': activeIndex } as React.CSSProperties}
              aria-hidden="true"
            />
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="workflow-detail-meta">
              <span><ShieldCheck aria-hidden="true" size={16} /> Status<strong>{active.status}</strong></span>
              <span><UserRound aria-hidden="true" size={16} /> Owner<strong>{active.owner}</strong></span>
              <span><FileText aria-hidden="true" size={16} /> Evidence<strong>{active.evidence}</strong></span>
            </div>
            <Link2 className="workflow-detail-link" aria-hidden="true" size={18} />
          </motion.div>
        </AnimatePresence>
      </div>
    </section>
