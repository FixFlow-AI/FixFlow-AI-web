...
export function Login() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      let mockRole: 'client' | 'freelancer' | 'agency' | 'developer' = 'client'
    ...
    <div className="min-h-screen bg-[#F7F8FA] flex flex-col justify-between py-12 px-6">
      ...
      <main className="max-w-md w-full mx-auto my-auto py-8">
        <div className="bg-white border border-[#D9E0E8] rounded-lg p-8 shadow-sm">
          <div className="mb-8">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Access Portal</span>
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Welcome back</h1>
            <p className="text-slate-500 text-sm mt-2">Enter your work credentials to open your project workspace.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <a href="#/login" onClick={(e) => e.preventDefault()} className="text-xs font-semibold text-[#2563EB] hover:text-[#173EA5]">
                  Forgot password?
                </a>
              </div>
              <div className="relative">
          ...
          <div className="mt-8 pt-6 border-t border-[#D9E0E8] flex items-center justify-between text-xs text-slate-500">
            <span className="flex items-center gap-1.5 font-medium text-slate-400">
              <ShieldCheck size={14} className="text-emerald-500" /> Secure SHA-256 session
            </span>
            <span>
              New here?{' '}
              <a href="#/signup" onClick={() => setPage('signup')} className="font-bold text-[#2563EB] hover:text-[#173EA5]">
                Request access
              </a>
            </span>
          </div>
