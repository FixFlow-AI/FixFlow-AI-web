...
const intelligenceShots: ProductShot[] = [
  {
...
const proposalAgreementShots: ProductShot[] = [
  {
...
const deliveryFundsShots: ProductShot[] = [
  {
...
function ProductFrame({ shot, priority = false }: { shot: ProductShot; priority?: boolean }) {
  return (
...
export function ProductOverviewMoment() {
  return (
    <section className="product-moment product-moment--overview" aria-labelledby="product-overview-title">
      <div className="section-shell product-moment-shell">
        <div className="product-moment-copy">
          ...
          <h2 id="product-overview-title">The whole project truth stays visible.</h2>
          <p>
            The first product signal shows FixFlowAI as a connected operating layer, not another
            profile marketplace. Brief, proof, agreement state, protected funds, and the next
            decision share one workspace.
          </p>
        </div>
        <ProductFrame
          priority
...
export function IntelligenceProductMoments() {
  return (
    <section className="product-moment product-moment--band" aria-labelledby="intelligence-products-title">
      <div className="section-shell product-moment-shell">
        <div className="product-moment-copy product-moment-copy--wide">
          <span className="product-moment-label">Before matching</span>
          <h2 id="intelligence-products-title">Clarity and proof arrive before anyone is shortlisted.</h2>
          <p>
            These screens make the invisible work visible: the brief is structured, evidence is
            traced, and the buyer can see exactly where confidence is strong or incomplete.
          </p>
        </div>
...
export function ProposalAgreementMoments() {
  return (
    <section className="product-moment" aria-labelledby="agreement-products-title">
      <div className="section-shell product-moment-shell">
        <div className="product-moment-copy product-moment-copy--wide">
          <span className="product-moment-label">From idea to agreement</span>
          <h2 id="agreement-products-title">The proposal is not a PDF. It is a living agreement path.</h2>
          <p>
            FixFlowAI turns early project context into a proposal structure, then hardens it into a
            working agreement with criteria, ownership, assumptions, and funding state.
          </p>
        </div>
...
export function DeliveryFundsMoments() {
  return (
    <section className="product-moment product-moment--band" aria-labelledby="delivery-products-title">
      <div className="section-shell product-moment-shell">
        <div className="product-moment-copy product-moment-copy--wide">
          <span className="product-moment-label">During execution</span>
          <h2 id="delivery-products-title">Progress, change requests, and funds stay in the same story.</h2>
          <p>
            The workroom keeps the agreement alive during delivery. When scope changes or acceptance
            conditions move, the impact is explicit before funds or reputation are affected.
          </p>
        </div>
        <div className="product-grid product-grid--duo">
    ...
    <section className="product-moment" aria-labelledby="outcome-product-title">
      <div className="section-shell product-moment-shell product-moment-shell--split">
        <div className="product-moment-copy">
          ...
          <h2 id="outcome-product-title">Reputation becomes a source-backed proof record.</h2>
          <p>
            Accepted work closes the loop. The outcome screen shows what was accepted, which sources
            support it, and how that proof can be reused without exposing private project details.
          </p>
        </div>
        <ProductFrame
          shot={{
...
export function RoleOnboardingMoment() {
  return (
    <section className="product-moment product-moment--pre-cta" aria-labelledby="role-product-title">
      <div className="section-shell product-moment-shell product-moment-shell--split">
        <div className="product-moment-copy">
          <span className="product-moment-label">Role-aware onboarding</span>
          <h2 id="role-product-title">Setup adapts without splitting into four products.</h2>
          <p>
            Clients, freelancers, agencies, and developers enter through the same trust system. The
            onboarding screen shows how role-specific evidence becomes future matching context.
          </p>
        </div>
        <ProductFrame
          shot={{
