...
export function HowItThinks() {
  ...
  useLayoutEffect(() => {
    ...
    if (!section || reducedMotion || window.matchMedia('(max-width: 899px)').matches) return undefined

    const context = gsap.context(() => {
      const steps = gsap.utils.toArray<HTMLElement>('[data-thinking-step]')
  ...
  return (
    <section className="thinking section-shell" id="thinking" ref={sectionRef}>
      ...
      <div className="thinking-layout">
        <div className="thinking-steps" role="tablist" aria-label="Execution reasoning stages">
          {thinkingSteps.map((step, index) => {
            const isActive = index === activeIndex
            return (
              <div className={`thinking-step${isActive ? ' is-active' : ''}`} key={step.label} data-thinking-step>
                <button
                  type="button"
                  role="tab"
        ...
        <div className="thinking-sticky">
          ...
          >
            <div className="thinking-canvas-top">
              ...
              <span>Stage 0{activeIndex + 1} / 0{thinkingSteps.length}</span>
              <span className="thinking-live"><i /> Reasoning visible</span>
            </div>
            ...
            <div className="thinking-progress" aria-hidden="true">
              <span style={{ width: `${((activeIndex + 1) / thinkingSteps.length) * 100}%` }} />
            </div>
          </motion.div>
        </div>
      </div>
