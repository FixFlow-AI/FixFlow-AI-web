...
function LeadCard({ lead, onSelect, onDragStart }) {
  return (
    <motion.button
      ...
      className="w-full rounded-lg border border-border/75 bg-transparent p-3 text-left transition-colors hover:border-primary/40 hover:bg-background/40 focus-within:bg-background/40"
    >
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {(lead.company.stack || []).slice(0, 3).map((item) => (
          <span key={item} className="rounded-full border border-border bg-transparent px-2 py-0.5 font-mono text-[10px] text-muted-foreground transition-colors hover:bg-card focus-within:bg-card">{item}</span>
        ))}
      </div>
    </motion.button>
  )
...
function LeadDetail({ lead, onClose, onDraft, onSend, isDrafting, isSending }) {
  ...
  return (
    <motion.aside
      ...
      className="fixed inset-y-0 right-0 z-50 w-full max-w-xl overflow-y-auto border-l border-border bg-transparent p-5 shadow-2xl transition-colors hover:bg-card focus-within:bg-card"
    >
      ...
      <div className="space-y-5 py-5">
        <TechnicalPanel className="p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(lead.company.stack || []).map((item) => (
              <span key={item} className="rounded-full border border-border bg-transparent px-2.5 py-1 font-mono text-xs text-muted-foreground transition-colors hover:bg-background/40 focus-within:bg-background/40">{item}</span>
            ))}
          </div>
        </TechnicalPanel>
        ...
        <TechnicalPanel className="p-4">
          ...
          <div className="mt-3 space-y-2">
            {(lead.reasoning || []).map((reason) => (
              <div key={reason} className="rounded-lg bg-transparent px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-background/40 focus-within:bg-background/40">{reason}</div>
            ))}
          </div>
        </TechnicalPanel>
        ...
        <TechnicalPanel className="p-4">
          ...
          <div className="mt-3 rounded-lg border border-border bg-transparent p-3 transition-colors hover:bg-background/50 focus-within:bg-background/50">
